...
public class ProductTagView extends LinearLayout {
    ...
    public ProductTagView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public ProductTagView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void setTag(String productTags, String type) {
        ...
        for (String tag : productTagArray) {
            ...
            imageView.setPadding(0,0 , ViewUtils.dip2px(context, 3), 0);
...
//            imageView.setTextColor(getTagTextColorRes(type,tag));
            imageView.setTextColor(ContextCompat.getColor(context,R.color.text_color1));
            addView(imageView, params);
        }
    }
    public void setTag1(String productTags, String type) {
        removeAllViews();
        if (TextUtils.isEmpty(productTags) || TextUtils.isEmpty(type)) {
            setVisibility(View.INVISIBLE);
            return;
        ...
        for (String tag : productTagArray) {
            ...
            TextView imageView = new TextView(context);
            imageView.setPadding(ViewUtils.dip2px(context, 5), 0, ViewUtils.dip2px(context, 5), 0);
            imageView.setTextSize(TypedValue.COMPLEX_UNIT_SP, 12);
            imageView.setText(tag);
            imageView.setBackgroundResource(R.drawable.tag_xs_bg);
            imageView.setTextColor(ContextCompat.getColor(context,R.color.tag_xs_color));
            addView(imageView, params);
        }
    }
    //获取tag背景
...
//            imageView.setTextColor(getListTagTextColorRes(type, tag));
    ...
    public void setListDarkTag(String productTags, String type) {
        ...
        String[] productTagArray = productTags.split(";");
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
        params.leftMargin = ViewUtils.dip2px(context, 0);
        for (String tag : productTagArray) {
            if(TextUtils.isEmpty(tag)){
                continue;
            }
